...
export default {
  ...
  methods: {
    ...
    svgMouseDown(e) {
      // this.$refs.mainBody[this.choice.index].nodePopFn();
      // svg鼠标按下触发事件分发
      this.setInitRect();
    ...
    linkEnd(i, nth) {
      ...
      let start = this.DataAll.nodes[this.choice.index].levels;
      let target = this.DataAll.nodes[i].levels;
      console.log(start,target)
      if(start[start.length-1]>=target[target.length-1]){
        this.$message({
          message: '同级或向上一级无法连线',
          type: 'warning'
        });
